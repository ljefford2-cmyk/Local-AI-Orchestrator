...
Two independent adversarial reviews of the personal gateway implementation surfaced nine structural concerns. A cross-domain reconciliation traced each finding across all implementations (federal, clinical, SMB, personal, public document) to determine whether the gaps were genuine architectural debt or documentation debt — answers that existed in other domains but weren't carried forward.
...
| 3 | Route-don't-reason boundary definition | **Partially addressed** | Consistent functional descriptions across all domains; no formal permitted/prohibited taxonomy existed until v3.0 |
...
**Design now, implement later:** Memory selection policy (Finding 2), route-don't-reason boundary (Finding 3), audit log two-stream design (Finding 6), multi-model dispatch deferral (Finding 9).
